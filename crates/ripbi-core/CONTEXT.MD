...
- **Report** (`src/report.rs`): the normalized `ReportModel` AST — pages, visuals,
  ...
  reachability-root surface; deleted-page bookmark sections stay in
  `Bookmark::stale_sections` for unused chains and never enumerate as roots.
- **Ingestion** (`src/ingest/`): detect and parse the source formats —
  `.pbix`/`.pbit` (unzip in-memory, `DataModelSchema` or the ABF `DataModel` +
  `Report/Layout` or PBIR), `.abf` backups (`src/ingest/abf/`: XPress9 container
  → backup log → `metadata.sqlitedb` → AST; metadata only — the storage catalog
  and logged file sizes become display-only `StorageStats` on tables, columns, and
  relationships in `abf/metadata/storage.rs`, issue #122), `.pbip` folders
  (`.Report/` PBIR + `.SemanticModel/` TMDL), standalone TMDL folders, and TMSL
  `model.bim`. All normalize into `TabularDatabase` + `ReportModel`. The XPress9
  codec lives in `crates/ripbi-xpress9`, the only crate allowed `unsafe` (FFI).
...
- **Graph** (`src/graph/`): build a DAG with `petgraph`, BFS reachability from report
  ...
  graph, the broken-binding records (`DependencyGraph::broken_bindings`) and
  DAX-owner findings (`DependencyGraph::broken_artifacts`, `graph/broken.rs`) — written
  references that resolve to nothing, or bindings onto broken artifacts (issues #60/#84).
  The precision bar inverts the conservatism rule:
...
| Broken bindings and artifacts | [docs/graph.md](docs/graph.md), [docs/name-resolution.md](docs/name-resolution.md) | The `broken_visual` and `broken_artifact` records: what counts as broken and what under-claims (KPI variants, variation hierarchies, calculated-table expressions, `Written`). Read `graph/broken.rs` before touching resolution-miss handling |
