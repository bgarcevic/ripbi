...
/// Runs `report` with in-memory streams and returns (exit code, stdout,
/// stderr). Stdin is non-interactive.
pub fn run_report(args: &ReportArgs, cwd: &Path, stdin: &str) -> (i32, String, String) {
    let mut out = Vec::new();
    let mut err = Vec::new();
    let mut input = stdin.as_bytes();
    let mut streams = Streams {
    ...
    let code = report::run_in(args, cwd, &mut streams);
    (
        code,
        String::from_utf8(out).expect("stdout is utf-8"),
        String::from_utf8(err).expect("stderr is utf-8"),
...
/// `report` with default flags against an explicit PATH.
pub fn report_path(path: &Path, cwd: &Path) -> (i32, String, String) {
    let args = ReportArgs {
        path: Some(path.to_path_buf()),
        ..ReportArgs::default()
    };
